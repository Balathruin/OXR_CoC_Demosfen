...
void CCarWeapon3::BoneCallbackX		(CBoneInstance *B)
{
	CCarWeapon3	*P = static_cast<CCarWeapon3*>(B->callback_param());
	Fmatrix rX;		rX.rotateX		(P->m_cur_x_rot);
	B->mTransform.mulB_43			(rX);
}

void CCarWeapon3::BoneCallbackY		(CBoneInstance *B)
{
	CCarWeapon3	*P = static_cast<CCarWeapon3*>(B->callback_param());
...
CCarWeapon3::CCarWeapon3(CPhysicsShellHolder* obj)
{
	m_bActive	= false;
	m_bAutoFire	= false;
	m_object	= obj;
	...
	m_rotate_x_bone			= K->LL_BoneID	(pUserData->r_string("mounted_weapon_definition3","rotate_x_bone3"));
	m_rotate_y_bone			= K->LL_BoneID	(pUserData->r_string("mounted_weapon_definition3","rotate_y_bone3"));
	m_fire_bone				= K->LL_BoneID	(pUserData->r_string("mounted_weapon_definition3","fire_bone3"));
	m_min_gun_speed			= pUserData->r_float("mounted_weapon_definition3","min_gun_speed3");
	m_max_gun_speed			= pUserData->r_float("mounted_weapon_definition3","max_gun_speed3");
	...
	Load								(pUserData->r_string("mounted_weapon_definition3","wpn_section3"));
	SetBoneCallbacks					();
...
CCarWeapon3::~CCarWeapon3()
{
	delete_data(m_Ammo);
//.	m_object->processing_deactivate		();
}

void CCarWeapon3::Load(LPCSTR section)
{
	inheritedShooting::Load(section);
	HUD_SOUND_ITEM::LoadSound(section,"snd_shoot3", m_sndShot, SOUND_TYPE_WEAPON_SHOOTING);
	HUD_SOUND_ITEM::LoadSound(section,"snd_shoot_high_3", m_sndShot_3, SOUND_TYPE_WEAPON_SHOOTING);
	m_Ammo->Load(pSettings->r_string(section, "ammo_class3"), 0);
}

void CCarWeapon3::UpdateCL()
{
	if(!m_bActive) return;
	UpdateBarrelDir				();
	IKinematics* K				= smart_cast<IKinematics*>(m_object->Visual());
...
void CCarWeapon3::Render_internal() { RenderLight(); }

void CCarWeapon3::SetBoneCallbacks()
{
...
void CCarWeapon3::ResetBoneCallbacks()
{
	CBoneInstance& biX		= smart_cast<IKinematics*>(m_object->Visual())->LL_GetBoneInstance(m_rotate_x_bone);	
	biX.reset_callback		();
	CBoneInstance& biY		= smart_cast<IKinematics*>(m_object->Visual())->LL_GetBoneInstance(m_rotate_y_bone);	
...
void CCarWeapon3::UpdateBarrelDir()
{
	IKinematics* K		= smart_cast<IKinematics*>(m_object->Visual());
...
bool CCarWeapon3::AllowFire() { return m_allow_fire; }

float CCarWeapon3::FireDirDiff()
{
	Fvector d1,d2;
...
const Fvector&	CCarWeapon3::get_CurrentFirePoint() { return m_fire_pos; }
const Fmatrix&	CCarWeapon3::get_ParticlesXFORM() { return m_fire_bone_xform; }
void CCarWeapon3::FireStart() { inheritedShooting::FireStart(); }

void CCarWeapon3::FireEnd()	
{
	inheritedShooting::FireEnd();
	StopFlameParticles	();
}

void CCarWeapon3::OnShot()
{
	...
	HUD_SOUND_ITEM::PlaySound	(m_sndShot_3, m_fire_pos, m_object, true);
}

void CCarWeapon3::Action				(u16 id, u32 flags)
{
	switch (id)
    {
        case eWpnFire:
...
void CCarWeapon3::SetParam(int id, Fvector2 val)
{
    switch (id)
    {
...
void CCarWeapon3::SetParam(int id, Fvector val)
{
    switch (id)
    {
...
const Fvector&	CCarWeapon3::ViewCameraPos() { return m_fire_pos; }
const Fvector&	CCarWeapon3::ViewCameraDir() { return m_fire_dir; }
const Fvector&	CCarWeapon3::ViewCameraNorm() { return m_fire_norm; }
