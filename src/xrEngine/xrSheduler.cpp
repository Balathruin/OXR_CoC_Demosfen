...
void CSheduler::ProcessStep()
{
    ...
    for (auto& entity : deferredUpdateInfo)
    {
        ...
        if (listOfItems.size() > sheduler_smoother_max_per_time)
        {
            ...
            Msg("* xrSheduler: jam detected and smoothed for range [%d-%d msec] (were %d items)", preferedFrame,
                preferedFrame + sheduler_smoother_frame_size, listOfItems.size(), neededDelayBtwnItems);
            u32 counter = 0;
